...
pub(super) fn encrypt ( rng : &SystemRandom, key : &[u8;256], mut plaintext : Vec<u8>) -> Result<(Vec<u8>, Vec<u8>), Unspecified> {
    // Convert key.
...
pub(super) fn decrypt ( key : &[u8;256], nonce : &[u8], mut ciphertext : Vec<u8>) -> Result<Vec<u8>, Unspecified> {
    // Convert key.
